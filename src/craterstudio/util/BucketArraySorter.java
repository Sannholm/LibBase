...
public class BucketArraySorter
{
    ...
    private final Bag<Sortable> less = new Bag<Sortable>();
    private Bag<Sortable>[] parts = new Bag[0];
    private final Bag<Sortable> more = new Bag<Sortable>();
    ...
    public void sort(Sortable[] p, int off, int len)
    {
        ...
        sortImpl(p, off, len, min, max, buckets);
    }
    ...
    private final void sortImpl(Sortable[] p, int off, int len, int min, int max, int buckets)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            if (index < min) less.put(s);
            else if (index >= max) more.put(s);
            else
            {
        ...
        if ((size = less.size()) != 0)
        {
            fetchSortStoreClear(p, off, less);
            off += size;
        }
        ...
        for (int i = 0; i < buckets; i++)
        {
            ...
            {
                fetchSortStoreClear(p, off, parts[i]);
                off += size;
            }
        }
        ...
        if ((size = more.size()) != 0)
        {
            fetchSortStoreClear(p, off, more);
            off += size;
        }
    }
