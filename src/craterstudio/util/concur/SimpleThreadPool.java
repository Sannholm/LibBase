...
public class SimpleThreadPool implements Executor
{
    ...
    public SimpleThreadPool(final String name, int minWorkers, int maxWorkers, long timeout, final long stackSize)
    {
        ...
        pump = new SimpleBlockingQueue<Runnable>();
        pool = new ThreadPoolExecutor(minWorkers, maxWorkers, timeout, TimeUnit.MILLISECONDS, new SynchronousQueue<Runnable>(), customStacksizeThreadFactory(name, stackSize));
        ...
        new Thread(new Runnable()
        {
            ...
            public void run()
            {
                for (Runnable task; (task = pump.take()) != null;)
                {
                    pool.execute(task);
                }
            }
    ...
    static ThreadFactory customStacksizeThreadFactory(final String name, final long stackSize)
    {
        return new ThreadFactory()
        {
            ...
            public Thread newThread(final Runnable task)
            {
                ...
                String tname = "SimpleThreadPool-" + name + "-thread-" + counter.incrementAndGet();
