...
public class ScheduledTaskQueue
{
    ...
    {
        tree = new TreeSet<ScheduledTask>(new ScheduledTaskComparator());
    ...
    public void schedule(Runnable task, long msDelay)
    {
        ...
        scheduleAt(task, Clock.now() + msDelay);
    }
    ...
    public void scheduleAt(Runnable task, long timestamp)
    {
        ...
        scheduleImpl(new ScheduledTask(task, timestamp));
    }
        ...
        Runnable loop = new Runnable()
        {
            ...
            public void run()
            {
                if (repeatCondition.accept(item))
                {
                    ...
                    scheduleImpl(new ScheduledTask(this, increment));
                }
            }
        };
        
        scheduleImpl(new ScheduledTask(loop, timestamp));
    }
