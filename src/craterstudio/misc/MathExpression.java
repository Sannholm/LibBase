...
public class MathExpression
{
    ...
    {
        return parse(expr).solve();
    }
    
    public double put(String var, double value)
    {
        double current = Double.NaN;
        Double found = vars.remove(var);
        if (found != null)
            current = found.doubleValue();
        vars.put(var, Double.valueOf(value));
        return current;
    }
    ...
    {
        CodeFeeder feeder = new CodeFeeder(vars);
    ...
    static void modifyByMathRules(Node node, int[][] operatorsPerLevel)
    {
        ...
        for (int level : levels)
            if (level != 0 && level != operatorCount)
                needsChange = true;
        ...
        public void feed(char c)
        {
            switch (c)
            {
                ...
                case '(':
                    Node next = new Node();
                    next.parent = node;
                    next.ci = new VariableOrValueInterpreter(this, mapping);
    ...
    static class VariableOrValueInterpreter extends CodeInterpreter
    {
        ...
        CodeInterpreter feed(char c)
        {
            if (!is(c))
            {
                feeder.node.elements.add(createVariableOrValue(String.valueOf(name, 0, index)));
                index = 0;
                return new OperatorInterpreter(feeder, mapping);
            }
        ...
        private Element createVariableOrValue(String str)
        {
            ...
            return new Variable(mapping, str);
        }
    ...
    static class OperatorInterpreter extends CodeInterpreter
    {
        ...
        CodeInterpreter feed(char c)
        {
            ...
            if (op == null)
                return new VariableOrValueInterpreter(feeder, mapping);
    ...
    static class Variable extends Element
    {
        ...
        {
            mapping.put(name, Double.valueOf(value));
        }
        
        @Override
        public double solve()
        {
            Double value = mapping.get(name);
            if (value == null)
