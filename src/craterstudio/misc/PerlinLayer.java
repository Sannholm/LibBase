...
public class PerlinLayer
{
    ...
    */
    ...
    public final float getSmooth1D(float x)
    {
        ...
        for (PerlinNoise element : noise)
            for (int octave = 1; octave <= octaves; octave++)
                height += element.noise1D(x0, octave);
    ...
    public final float getSmooth2D(float x, float y)
    {
        ...
        for (PerlinNoise element : noise)
            for (int octave = 1; octave <= octaves; octave++)
                height += element.noise2D(x0, y0, octave);
    ...
    public final float getSmooth3D(float x, float y, float z)
    {
        ...
        for (PerlinNoise element : noise)
            for (int octave = 1; octave <= octaves; octave++)
                height += element.noise3D(x0, y0, z0, octave);
    ...
    public final float getRough2D(float x, float y)
    {
        ...
        for (PerlinNoise element : noise)
        {
            float h = 0.0f;
            for (int octave = 1; octave <= octaves; octave++)
                h += element.noise2D(x0, y0, octave);
            if (h < 0.0f)
                h *= -1.0f;
            height += h;
        }
    ...
    public final float getRough3D(float x, float y, float z)
    {
        ...
        for (PerlinNoise element : noise)
        {
            float h = 0.0f;
            for (int octave = 1; octave <= octaves; octave++)
                h += element.noise3D(x0, y0, z0, octave);
            if (h < 0.0f)
                h *= -1.0f;
            height += h;
        }
