...
public class Json
{
    ...
    private static Map<String, Object> unpackObject(JSONObject obj) throws JSONException
    {
        ...
        for (Object key : IteratorUtil.foreach(obj.keys()))
        {
            ...
            if (got instanceof JSONObject) got = unpackObject((JSONObject)got);
    ...
    private static List<Object> unpackArray(JSONArray arr) throws JSONException
    {
        ...
        {
            if (arr.get(i) instanceof JSONObject) list.add(unpackObject(arr.getJSONObject(i)));
            else if (arr.get(i) instanceof JSONArray) list.add(unpackArray(arr.getJSONArray(i)));
            else
                list.add(arr.get(i));
        }
        return list;
