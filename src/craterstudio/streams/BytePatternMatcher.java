...
public abstract class BytePatternMatcher
{
    ...
    {
        final byte[] dataBuffer = new byte[pattern.length];
        ...
        while (true)
        {
            ...
            if (indexOf == 0)
            {
                ...
                onPattern(out, dataBuffer, 0, pattern.length);
                writeOffset = consume(out, dataBuffer, writeOffset, pattern.length, false);
            }
            else if (indexOf == -1)
            {
                // nothing found
