...
public class FileMonitor
{
    ...
    */
    ...
    public void check()
    {
        if (firstTime)
        {
            if (startWithCreateEvent) previousFile = null;
            else
                previousFile = wrap(file);
            
            firstTime = false;
        }
        
        // get contents of dir
        MyFile currentFile = wrap(file);
        
        // compare
        
        switch (isNew(currentFile, previousFile))
        {
            case -1:
                switch (isUpd(currentFile, previousFile))
                {
                    case -1:
                        break;
                    case 0:
                        fireFileUpdatingEvent(currentFile.backing);
                        ignoreUpdate(currentFile, previousFile);
                        break;
                    case +1:
                        fireFileUpdatedEvent(currentFile.backing);
                        break;
                }
                break;
            ...
            case +1:
                fireFileCreatedEvent(currentFile.backing);
                break;
        }
        ...
        {
            fireFileDeletedEvent(currentFile.backing, currentFile.isDir);
        }
