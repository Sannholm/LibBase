...
public class Spline2D
{
    ...
    public void enabledTripCaching(float maxTravelStep, float positionStep)
    {
        ...
        posStep = positionStep;
        
        float x = this.x[0].eval(0.0f);
        float y = this.y[0].eval(0.0f);
        
        travelCache = new ArrayList<CacheItem>();
        travelCache.add(new CacheItem(x, y, 0.0f));
    }
    ...
    {
        if (length < 0.0f)
            length = getSteppingPosition(0.0f, Integer.MAX_VALUE, stepSize).travelled;
        return length;
    }
    ...
    public boolean getTripPosition(float totalTrip, float[] coords)
    {
        ...
        CacheItem last = travelCache.get(travelCache.size() - 1);
        ...
        while (last.travelled < totalTrip)
        {
            ...
            CacheItem curr = getSteppingPosition(last.position, travel, posStep);
            
            if (curr.position >= count)
            {
            ...
            if (curr.travelled > maxTravelStep * 0.95f)
            {
                travelCache.add(curr);
            }
        ...
        int lo = 0;
        int hi = travelCache.size() - 1;
        
        while (true)
        {
            int mid = (lo + hi) / 2;
            
            last = travelCache.get(mid);
        ...
        for (int i = lo; i <= hi; i++)
        {
            CacheItem item = travelCache.get(i);
        ...
        last = getSteppingPosition(last.position, travel, posStep);
        coords[0] = last.xpos;
        coords[1] = last.ypos;
    ...
    private CacheItem getSteppingPosition(float positionOffset, float travel, float positionStep)
    {
        ...
        while (totalTravelled < travel && currentPosition < count)
        {
